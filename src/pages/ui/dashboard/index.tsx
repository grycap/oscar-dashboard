...
const Dashboard = () => {
  ...
  const memTotal = data.detail.reduce((acc, node) => acc + parseInt(node.memoryCapacity), 0);
    ...
    <div className="w-full max-w-full mx-auto px-4 pt-6 pb-6 space-y-6">
      ...
      <Card className="w-full">
        <CardHeader>
          ...
          <CardDescription className="text-sm text-gray-500">
            Detailed information about the nodes. Currently  {data.detail.length} active node(s).
          </CardDescription>
        </CardHeader>
